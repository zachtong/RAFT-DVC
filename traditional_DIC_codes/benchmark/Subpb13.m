...
%% [BENCHMARK SHADOW COPY] of ALDVC\func\Subpb13.m
%% Purpose: headless benchmarking. Changes vs original:
%%   1) waitbar/parfor_progressbar calls removed (zero numerical impact)
%%   2) 'case 0 || 1' switch bug fixed -> 'case {0,1}' (clusterNo>1 = parfor)
%%   3) parfor branch: preallocate UtempPar/VtempPar/WtempPar (original
%%      preallocated *2-suffixed names by mistake) and fix undefined
%%      ConvItPerEle2 reference that breaks parfor static analysis
%% Original algorithm and numerics are otherwise UNCHANGED.
...
switch clusterNo
    ...
    otherwise
        ...
        temp = zeros(size(coordinatesFEM,1),1); UtempPar = temp; VtempPar = temp; WtempPar = temp;
        ConvItPerEle = zeros(size(coordinatesFEM,1),1); 
        tic; 
        ...
        parfor tempjj = 1:size(coordinatesFEM,1)
            ...
            if displayIterOrNot == 1, disp(['ele ',num2str(tempjj),' converge or not is ',num2str(ConvItPerEle(tempjj)),' (1-converged; 0-unconverged)']); end
