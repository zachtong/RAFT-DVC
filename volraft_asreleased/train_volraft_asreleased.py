...
``bilinear_sampler`` axis-order bug FIXED (see volraft_asreleased/model/utils/utils.py).
...
Output: C:/Zixiang_local_data/raft-dvc/paper1/phase1/volraft_asreleased_r4md64/
      (training.log, latest.pth, best_model.pth, checkpoint_epoch_*.pth --
      written by src.training.Trainer).

Launch instructions: volraft_asreleased/LAUNCH.md
"""
...
# -- Make repo root importable (this file lives at <repo>/volraft_asreleased/) ----
...
from volraft_asreleased.model.volraft import ModuleVolRAFT  # noqa: E402

DEFAULT_DATA_DIR = Path(r"G:\Zixiang_local_data\raft-dvc\data_paper1_v3\r4_medium_size64")
DEFAULT_OUTPUT_DIR = Path(r"C:\Zixiang_local_data\raft-dvc\paper1\phase1\volraft_asreleased_r4md64")
...
class VolRAFTFixed(nn.Module):
    ...
    ARCH = {
        "network_name": "volraft_asreleased",
        ...
        "hidden_dim": 96,
        "context_dim": 64,
        "sampler": "fixed (see volraft_asreleased/model/utils/utils.py)",
    }
...
def main() -> None:
    ...
    print("=" * 72)
    print(f"[volraft_asreleased] data       : {data_dir}")
    print(f"[volraft_asreleased] output     : {output_dir}")
    print(f"[volraft_asreleased] batch size : {batch_size} "
          f"({steps_per_epoch} optimizer steps/epoch, "
          f"{args.epochs * steps_per_epoch} total)")
    print(f"[volraft_asreleased] params     : {model.get_num_parameters():,}")
    print(f"[volraft_asreleased] amp        : {mixed_precision}")
    print(f"[volraft_asreleased] cuda       : {torch.cuda.is_available()}")
    print("=" * 72, flush=True)
    ...
    print(f"[volraft_asreleased] done. best_val_loss={best_val:.6f}")
    print(f"[volraft_asreleased] checkpoints at: {output_dir}")
