...
class RAFTDVCConfig:
    ...
    def __post_init__(self):
        ...
        if self.corr_impl not in ("standard", "on_the_fly", "cuda_otf", "cuda_otf_v3", "cuda_otf_v4"):
            raise ValueError(
                f"Invalid corr_impl: {self.corr_impl!r}. "
                f"Must be one of: 'standard', 'on_the_fly', 'cuda_otf', 'cuda_otf_v3', 'cuda_otf_v4'"
            )
...
class RAFTDVC(nn.Module):
    ...
    def forward(
        ...
        if self.config.corr_impl == "cuda_otf_v4":
