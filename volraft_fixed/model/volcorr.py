...
class CorrBlock:
    ...
    def __call__(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            delta = torch.stack(torch.meshgrid(dy, dx, dz, indexing='ij'), axis=-1)

            centroid_lvl = coords.reshape(batch * h1 * w1 * d1, 1, 1, 1, nf) / 2**i
            delta_lvl = delta.view(1, 2*r+1, 2*r+1, 2*r+1, nf)
            coords_lvl = centroid_lvl + delta_lvl

            corr = bilinear_sampler(corr, coords_lvl)
            corr = corr.view(batch, h1, w1, d1, -1)
    ...
    def corr(fmap1, fmap2):
        ...
        fmap1 = fmap1.view(batch, dim, ht * wd * dp)
        fmap2 = fmap2.view(batch, dim, ht * wd * dp) 
